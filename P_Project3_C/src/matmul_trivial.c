...
struct TrivialMatrix* trivial_matrix_addition_simd(const struct TrivialMatrix* matrixA, const struct TrivialMatrix* matrixB) {
    return matrix_element_by_element_linear_combinations_simd(matrixA, matrixB, 1, 1);
}
struct TrivialMatrix* matrix_element_by_element_linear_combinations_simd(const struct TrivialMatrix* matrixA,
    ...
    float _alphas[8] = { alpha, alpha, alpha, alpha, alpha, alpha, alpha, alpha };
    float _betas[8] = { beta, beta, beta, beta, beta, beta, beta, beta };
    __m256 alphas = _mm256_load_ps(_alphas);
    __m256 betas = _mm256_load_ps(_betas);
    //    for(int i = 0; i < matrixA->rowCount; i++) {
    //        for(int j = 0; j < matrixA->columnCount; j+=8) {
    //            __m256 elementA = _mm256_mul_ps(_mm256_load_ps(&TrivialMatrixGetElement(matrixA, i, j)), alphas);
    //            __m256 elementB = _mm256_mul_ps(_mm256_load_ps(&TrivialMatrixGetElement(matrixB, i, j)), betas);
    //            _mm256_store_ps(&TrivialMatrixGetElement(result,i, j), _mm256_add_ps(elementA, elementB));
    //        }
    //    }
    for(int j = 0; j < matrixA->columnCount; j++) {
        for(int i = 0; i < matrixA->rowCount; i += 8) {

            __m256 elementA = _mm256_mul_ps(
                _mm256_set_ps(TrivialMatrixGetElement(matrixA, i, j), TrivialMatrixGetElement(matrixA, i + 1, j),
                              TrivialMatrixGetElement(matrixA, i + 2, j), TrivialMatrixGetElement(matrixA, i + 3, j),
                              TrivialMatrixGetElement(matrixA, i + 4, j), TrivialMatrixGetElement(matrixA, i + 5, j),
                              TrivialMatrixGetElement(matrixA, i+6, j), TrivialMatrixGetElement(matrixA, i+7, j)),
                alphas);
            __m256 elementB = _mm256_mul_ps(
                _mm256_set_ps(TrivialMatrixGetElement(matrixB, i, j), TrivialMatrixGetElement(matrixB, i + 1, j),
                              TrivialMatrixGetElement(matrixB, i + 2, j), TrivialMatrixGetElement(matrixB, i + 3, j),
                              TrivialMatrixGetElement(matrixB, i + 4, j), TrivialMatrixGetElement(matrixB, i + 5, j),
                              TrivialMatrixGetElement(matrixB, i+6, j), TrivialMatrixGetElement(matrixB, i+7, j)),
                betas);
            float buffer[8];
            _mm256_store_ps(buffer, _mm256_add_ps(elementA, elementB));
            TrivialMatrixGetElement(result,i, j) = buffer[0];
            TrivialMatrixGetElement(result,i+1, j) = buffer[1];
            TrivialMatrixGetElement(result,i+2, j) = buffer[2];
            TrivialMatrixGetElement(result,i+3, j) = buffer[3];
            TrivialMatrixGetElement(result,i+4, j) = buffer[4];
            TrivialMatrixGetElement(result,i+5, j) = buffer[5];
            TrivialMatrixGetElement(result,i+6, j) = buffer[6];
            TrivialMatrixGetElement(result,i+7, j) = buffer[7];
        }
    }
    return result;
}
